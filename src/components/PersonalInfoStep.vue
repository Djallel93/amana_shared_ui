...
function update<K extends keyof PersonalInfoValue>(field: K, value: PersonalInfoValue[K]): void {
    emit('update:modelValue', { ...props.modelValue, [field]: value });
}

const afficherTelephoneBis = computed(() => props.showTelephoneBis && !!props.labels.telephoneBis);
</script>

<template>
    <div class="space-y-3">
        <div>
            <label class="block text-xs font-semibold text-ink mb-1">{{ labels.nom }} *</label>
            <input
        ...
        <div>
            <label class="block text-xs font-semibold text-ink mb-1">{{ labels.prenom }} *</label>
            <input
        ...
        <div>
            <label class="block text-xs font-semibold text-ink mb-1">{{ labels.telephone }} *</label>
            <input
                :value="modelValue.telephone"
                @input="update('telephone', ($event.target as HTMLInputElement).value)"
                type="tel" placeholder="0123456789"
                class="w-full px-3 py-2.5 border border-ink-faint rounded-md text-[14px] bg-surface-2 outline-none focus:border-accent">
            <p v-if="labels.telephoneHint" class="text-[11px] text-ink-faint mt-1">{{ labels.telephoneHint }}</p>
            <span v-if="errors.telephone" class="block text-[11px] text-rose-600">{{ errors.telephone }}</span>
        </div>

        <div v-if="afficherTelephoneBis">
            <label class="block text-xs font-semibold text-ink mb-1">{{ labels.telephoneBis }}</label>
            <input
                :value="modelValue.telephone_bis ?? ''"
                @input="update('telephone_bis', ($event.target as HTMLInputElement).value)"
                type="tel" placeholder="0123456789"
                class="w-full px-3 py-2.5 border border-ink-faint rounded-md text-[14px] bg-surface-2 outline-none focus:border-accent">
        ...
        <div>
            <label class="block text-xs font-semibold text-ink mb-1">{{ labels.email }} *</label>
            <input
